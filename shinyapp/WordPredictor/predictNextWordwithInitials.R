#---------------------------------------------------------------------------------------
# filename: predictNextWordwithInitials
# author: Luis Padua
# summary:
#       it grabs the user text input, since it found a letter as last character
...
predictNextWordwithInitials <- function(input) {
        ...
        ind <- grep(paste("^", input$token[length(input$token)-3], " ",
                          input$token[length(input$token)-2], " ",
        ...
        if (length(a) < 3) {
                ...
                if (length(a) < 3) {
                        ind <- grep(paste("^", input$token[length(input$token)-1], " ",
                                          input$token[length(input$token)], sep = ""), bigramDTM$dimnames$Terms)
